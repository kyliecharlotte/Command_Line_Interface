...
int count_token (char* buf, const char* delim)
{
	...
	*/
	...
	int count = 0;
	char* buf_copy = (char *)malloc(sizeof(char)*strlen(buf)+1);
	strcpy(buf_copy, buf);
	buf_copy[strlen(buf)] = '\0';

	token = strtok_r(buf_copy, delim, &ptr);
...
command_line str_filler (char* buf, const char* delim)
{
	...
	*/
	...
	char* ptr;
	char* token = strtok_r(buf_copy, delim, &ptr);
	char* curr_token;
	int count = 0;

	while (token != NULL) {

		curr_token = strdup(token);

		char* ptr_1;

		char* token_split = strtok_r(curr_token, delim, &ptr_1);

		while (token_split != NULL) {
			
			cl.command_list[count] = (char*)malloc(strlen(token_split) + 1);
			strcpy(cl.command_list[count], token_split);
			count++;
			token_split = strtok_r(NULL, delim, &ptr_1);
		}
		token = strtok_r(NULL, delim, &ptr);
		free(curr_token);
	}
